...
export default function usePayStripeLinkPayment(getPaymentIntent: any, confirmPaymentMethod: any, successCallback: any) {
    const {
        confirmCardPayment,
    ...
    const createPaymentRequest = useCallback(async (label: string, amountInCents: any) => {
        console.log('Creating Link Pay request: ', label, amountInCents);
        const pr = await (async () => {
            if (paymentRequestRef.current) {
                ...
                console.log('Link Pay availability:', result);
                if (!result || !result.link) {
                    console.error("Link Pay is not available.", result);

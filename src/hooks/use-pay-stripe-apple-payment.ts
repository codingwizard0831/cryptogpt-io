...
export default function usePayStripeApplePayment(getPaymentIntent: any, confirmPaymentMethod: any, successCallback: any) {
    const {
        confirmCardPayment,
    ...
    const _OnPaymentMethod = useCallback(async (e: any) => {
    ...
    const createPaymentRequest = useCallback(async (label: string, amountInCents: any) => {
        console.log('test1: ', label, amountInCents)
        const pr = await (async () => {
            if (paymentRequestRef.current) {
                ...
                if (!result || !result.applePay) {
                    console.error("ApplePay is not available.", result);

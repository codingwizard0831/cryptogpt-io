...
interface DeliverRestaurantsCarouselByNameProps extends BoxProps {
    title: string;
};

export default function DeliverRestaurantsCarouselByName({ title, sx, ...other }: DeliverRestaurantsCarouselByNameProps) {
            ...
            >
                <Carousel ref={restaurantCarousel.carouselRef} {...restaurantCarousel.carouselSettings}>
                    {dummyDataRestaurants.map((_, index) => (
                        <DeliverRestaurantItem data={_} key={`restaurent${index}`} />
                    ))}
                    {

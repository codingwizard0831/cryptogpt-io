...
const ScrollDown = React.forwardRef<HTMLDivElement, Props>(({ sx }, ref) => {
    const controls = useAnimation();

    const checkScroll = useCallback(() => {
        const currentRef = (ref as React.MutableRefObject<HTMLDivElement | null>).current;
        const isScrolled = currentRef ? currentRef.scrollTop < currentRef.scrollHeight - currentRef.clientHeight : false;
        if (isScrolled) {
    ...
    const handleClick = () => {
        const currentRef = (ref as React.MutableRefObject<HTMLDivElement | null>).current;
        if (currentRef) {
            currentRef.scrollTo({ top: currentRef.scrollHeight, behavior: 'smooth' });
        }
    };

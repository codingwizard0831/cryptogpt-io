...
export async function GET(req: NextRequest) {
    try {
        ...
        const { data, error } = await supabase.from('followers').select('*').eq('status', true);
        if (error) return NextResponse.json({ error: error.message }, { status: 500 })
        return NextResponse.json(data)
    } catch (error) {
    ...
    try {
        ...
        if (existingData) {
            ...
            return NextResponse.json({
                message: newStatus ? 'User followed successfully' : 'User unfollowed successfully',
                data
            });
        }
        ...
        return NextResponse.json({ error: 'Failed to manage follow relationship' }, { status: 500 })
    }
}